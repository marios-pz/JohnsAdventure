...
class Inventory:
    def __init__(self, screen: pygame.surface.Surface, sprite_sheet: Any):
        ...
        self.font = pygame.font.Font(
            resource_path("data/database/menu-font.ttf"), 18
        )

        # inventory button
        self.button_inv = scale(self.spr_sh.parse_sprite("inventory"), 3)

        self.button_inv_h = scale(
            self.spr_sh.parse_sprite("inventory_hover"), 3
        )
        ...
        self.inv_menu = pygame.Surface(
            (self.w // 3, self.h // 3), pygame.SRCALPHA
        )
        self.im_rect = self.inv_menu.get_rect(
            right=self.w - 25, y=160
        )  # center
        ...
        self.uii_rect = self.ui_inv.get_rect(
            right=self.inv_menu.get_width(), y=0
        )
        self.hover_color = self.ui_inv.get_at((20, 20))
        self.hover_color = [
            self.hover_color[0] - 25,
            self.hover_color[1] - 25,
            self.hover_color[2] - 25,
        ]
    ...
    def update(self, parent_class):
        ...
        if (
            ...
            for index in range(item_l):  # loop through the items
                ...
                if (
                    index * item.image.get_height() + 16
                ...
                item.update(
                    ...
                    (
                        self.uii_rect.x + 25,
                        index * item.image.get_height() + 12,
                    ),
                    damages,
            ...
            self.screen.blit(
                self.font.render(f"{item_l}/32", True, (0, 0, 0)),
                (self.im_rect[0] + 340, self.im_rect[1] + 175),
            )
            self.screen.blit(
                self.font.render(
                    f"HP Potions:{parent_class.health_potions}",
                    True,
                    (0, 0, 0),
                ),
    ...
    def handle_clicks(self, pos, player):
        ...
        if self.show_menu:
            pos -= pygame.Vector2(
                *self.im_rect.topleft
            )  # get the pos of the click on the surface
            for item in self.items:
                changes = item.handle_clicks(
                    pos, player
