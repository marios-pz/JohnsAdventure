...
class UpgradeStation:
    ...
    def update(self, parent_class):
        ...
        if self.show_menu:  # if it shows the menu, then it does not show the down bar
            ...
            self.up_st_menu.blit(self.txt_pt_av,
                                 self.txt_pt_av.get_rect(bottomright=self.ui_inv.get_size() + pg.Vector2(12, -10)))
...
class StatHandler:
    ...
    def update_value(self, value, item_addition: int = None):
        ...
        self.image = pg.Surface((self.text.get_width() + self.addition.get_width(), self.text.get_height()),
                                pg.SRCALPHA)
        self.rect = self.image.get_rect()

    def update(self, surface: pg.Surface, pos: tuple[int, int], add_args=(0, 0)):
...
class CritChance(StatHandler):
    ...
    def __init__(self, value: float):
        super().__init__("critical_chance", "Crit. Chance", value * 100, val_type=float, optional_unit="%",
                         color=(255, 255, 0), increment=0.01)
