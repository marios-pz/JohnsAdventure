...
class GameManager:
    ...
    def __init__(self, debug=False, first_state="player_room", no_rect=False):
        ...
        #
        self.font = pygame.font.Font(resource_path("data/database/menu-font.ttf"), 24)

        # (main menu title)
        self.blacksword = pygame.font.Font(resource_path("data/database/Blacksword.otf"), 113)
        ...
        # text + font
        self.title_font = pygame.font.Font(resource_path("data/database/menu-font.ttf"), 75)
        self.subtitle_font = pygame.font.Font(resource_path("data/database/menu-font.ttf"), 15)
        self.end_game_ui_texts = [
            self.title_font.render("DEATH", True, (255, 0, 0)),
            self.subtitle_font.render(f"Press {pygame.key.name(self.player.data['controls']['interact'])} "
        ...
        # font
        self.new_level_font = pygame.font.Font(resource_path("data/database/menu-font.ttf"), 30)
    ...
    def routine(self):
        ...
        if not self.menu and not self.loading and not self.death_screen:  # if the game is playing
            if self.state != "credits":
                user_interface(self.player, pygame.mouse.get_pos(), (
            ...
            self.DISPLAY.blit(self.player.mouse_icon, pygame.mouse.get_pos())
            self.quest_manager.update_quests()

            if self.showing_nl_popup:
                self.DISPLAY.blit(self.new_level_popup, self.new_level_popup_rect)
                time_elapsed = pygame.time.get_ticks() - self.begin_new_level_popup
...
class Debugging:
    ...
    def __init__(self, display, game_instance, no_rect):
        ...
        self.font = pygame.font.Font(resource_path("data/database/menu-font.ttf"), 15)
            ...
            if not self.game.menu and not self.game.loading:
                ...
                for obj in objects:
                    ...
                    if type(obj) is pygame.Rect:
                        pygame.draw.rect(self.screen, self.colors["collision_rect"],
                                     pygame.Rect(obj.topleft - self.player.camera.offset.xy, obj.size), 2)
                    else:
                        for key, color in self.colors.items():
                            if hasattr(obj, key):
                                attr = (getattr(obj, key))
                                if type(attr) is pygame.Rect:
                                    pygame.draw.rect(self.screen, color, attr, 1)
                        ...
                        if hasattr(obj, "IDENTITY"):
                            ...
                            if hasattr(obj, "d_collision"):
                                col_rect.topleft += pygame.Vector2(*obj.d_collision[:2])
                                col_rect.size = obj.d_collision[2:]
                            pygame.draw.rect(self.screen, self.colors["collision_rect"], col_rect, 2)
                ...
                for exit_rect in exit_rects:
                    ...
                    r.topleft -= scroll
                    pygame.draw.rect(self.screen, self.colors["exit_rect"], r, 2)
                ...
                pl_col_rect.w -= 70
                pl_col_rect.h -= 115
                pygame.draw.rect(self.screen, self.colors["collision_rect"], pl_col_rect, 1)

                try:
                    pygame.draw.rect(self.screen, (255, 0, 0), self.player.attacking_hitbox, 2)
                except TypeError:
                    pass

                position = (self.player.rect.topleft - self.player.camera.offset.xy)
                itr_box = pygame.Rect(*position, self.player.rect.w // 2, self.player.rect.h // 2)
